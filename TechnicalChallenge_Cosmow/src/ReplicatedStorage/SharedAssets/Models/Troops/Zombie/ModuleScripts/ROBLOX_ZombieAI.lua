...
--				end
	...
	CanSeeTarget.Evaluate = function()
		if model then
			...
			local target = AIUtilities:GetClosestVisibleTarget(model, characters, {}, configs["FieldOfView"])
			if target then
				ZombieTarget = target
				return true
			end
	...
	local function handleHit(other)
		if canHit then
			if other and other.Parent and other.Parent:FindFirstChild("Humanoid") then
				...
				local npcModelOwner = model:GetAttribute("Owner")
				local characterOwner = character:GetAttribute("Owner")

				if (npcModelOwner == character.Name) or (character:GetAttribute("NPC") and characterOwner == npcModelOwner) then
					return
				end
				
				local enemy = other.Parent						
				if enemy.Humanoid.WalkSpeed > 0 then
					enemy.Humanoid:TakeDamage(model:GetAttribute("Damage"))
					canHit = false
				end
			end
		else
